 ...
 * 運動図の意味と色の対応（全運動図で共通）
 *
 * 運動の図では「いまの姿勢」と「目指す姿勢」が同じ絵の中に並ぶため、
 * 色が図ごとに違うと、患者さんは毎回どちらが目標なのか読み直すことになる。
 * そこで意味ごとに色を固定し、図をまたいで同じ読み方ができるようにする。
 *
 *   開始姿勢 = 薄いグレー（これから動かす前の形）
 *   終了姿勢 = 濃い紺（この形になるのが目標）
 *   動作矢印 = 青（動かす向き）
 *   注意点   = オレンジ（やってはいけない・気をつける）
 *
 * 白黒印刷でも区別できるよう、開始姿勢は破線、終了姿勢は実線で描く。
 */
export const MOTION = {
  /** 開始姿勢（動く前）。薄いグレーの破線で描く */
  start: '#b6bfc9',
  /** 終了姿勢（目標の形）。濃い紺の実線で描く */
  end: '#16283d',
  /** 動作の向きを示す矢印と、その説明 */
